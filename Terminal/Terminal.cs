...
namespace Terminal {
	public class Terminal : TextBox {
		...
		protected virtual void HandleCommandHistoryRequest(CommandHistoryDirection direction) 
		{
			switch (direction) {
				case CommandHistoryDirection.Backward:
					...
					{
					    var command = CommandLog[IndexInLog].Raw;
                        while(command == "" && IndexInLog >= 0)
                        {
                            IndexInLog--;
                            command = CommandLog[IndexInLog].Raw;
                        }
				...
				case CommandHistoryDirection.Forward:
					...
					if (CommandLog.Count > 0) {
					    var command = CommandLog[IndexInLog].Raw;
                        while(command == "" && IndexInLog < CommandLog.Count - 1)
                        {
                            IndexInLog++;
                            command = CommandLog[IndexInLog].Raw;
                        }
        ...
        public int IndexInLog { get { return IndexInLog; }
            set { IndexInLog = value; }}
        ...
        //    //var completionManager = new CompletionManager(line);
        //    //completionManager.DoStuff();
